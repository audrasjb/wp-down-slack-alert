...
function wpdsa_add_sub_menu() {
	add_submenu_page(
		'tools.php',
		esc_html__( 'WP Down Slack Alert settings', 'wp-down-slack-alert' ),
		esc_html__( 'WP Down Slack Alert settings', 'wp-down-slack-alert' ),
		'manage_options',
	...
	<div class="wrap">
		...
		<form method="post">
			...
			<table class="form-table">
				<tr>
					<th>
						<label for="notification_token"><?php esc_html_e( 'Slack team token', 'wp-down-slack-alert' ); ?></label>
					</th>
					<td>
						<input style="width: 500px;" id="notification_token" name='notification_token' type="text" value="<?php echo esc_html( $token ); ?>"/>
						<p>
							<button type="button" class="button-secondary" id="toggle-slack-token-help" data-toggle="Close help">
								<?php esc_html_e( 'Generate a token for your Slack team', 'wp-down-slack-alert' ); ?>
							</button>
						</p>
						<div id="slack-token-help">
							<p><?php esc_html_e( 'It only takes few minutes!', 'wp-down-slack-alert' ); ?></p>
							<ol>
								<li><?php esc_html_e( 'Create a Slack App:', 'wp-down-slack-alert' ); ?>
									<ol>
										<li>
											<?php esc_html_e( 'Go to', 'wp-down-slack-alert' ); ?> <a href="https://api.slack.com/apps?new_app=1"><?php esc_html_e( 'this page', 'wp-down-slack-alert' ); ?></a>
										</li>
										<li><?php esc_html_e( 'Enter a Slack App Name, for example "My super WP notification bot"', 'wp-down-slack-alert' ); ?></li>
										<li><?php esc_html_e( 'Choose a slack Workspace', 'wp-down-slack-alert' ); ?></li>
									</ol>
								</li>
								<li><?php esc_html_e( 'Add a bot to your Slack App:', 'wp-down-slack-alert' ); ?>
									<ol>
										<li><?php esc_html_e( 'Choose "Bots"', 'wp-down-slack-alert' ); ?></li>
										<li><?php esc_html_e( 'Next up, click on "Add a Bot User" button', 'wp-down-slack-alert' ); ?></li>
										<li><?php esc_html_e( 'Leave the default names (you will be able to override that in the plugin settings), and click "Add bot user" button.', 'wp-down-slack-alert' ); ?></li>
									</ol>
								</li>
							</ol>
							<a href="https://get.slack.help/hc/fr-fr/articles/115005265703-Cr%C3%A9er-un-bot-pour-votre-espace-de-travail#cr%C3%A9er-un-bot" target="_blank"><?php esc_html_e( 'Create Slack app and token', 'wp-down-slack-alert' ); ?></a> <?php esc_html_e( 'And use: Bot User OAuth Access Token', 'wp-down-slack-alert' ); ?>
						</div>
						<script>
							jQuery('#slack-token-help').hide();
							jQuery('#toggle-slack-token-help').click(function (e) {
								e.preventDefault();
								var toggle = jQuery(this).attr('data-toggle');
								jQuery(this).attr('data-toggle', jQuery(this).html());
								jQuery(this).html(toggle);
								jQuery('#slack-token-help').slideToggle();
							});
						</script>
					</td>
				</tr>
			...
			<table class="form-table">
				<tr>
					...
					<td>
						...
						<script>
							jQuery('.rtms_visually_hidden').hide();
							jQuery('#notification_bot_disable_email').change(function () {
								jQuery('#tr_mail_recurrence').slideToggle();
							});
						</script>
					</td>
				</tr>
				...
				<tr>
					...
					<td>
						...
						<div class="image-preview-wrapper">
							<?php
							$image_src = 'https://www.roumazeilles.net/images/2007/hal_9000.gif';
			...
			<table class="form-table">
				<tr>
					...
					<td>
						<input type="text" id="notification_message_title" name="notification_message_title" class="regular-text" value="<?php echo esc_attr( get_option( 'settings_slack_notification_message_title' ) ); ?>"/>
					</td>
				</tr>
			</table>
			<p class="submit">
...
function wpdsa_send_slack_notification( $message, $error ) {
	...
	if ( true === $connexion_status['connected'] ) {
		if ( $new_value > ( $data + $recurrence ) ) {
			...
			$icon_url      = ( isset( $img ) && '' !== $img ) ? $img : 'https://www.roumazeilles.net/images/2007/hal_9000.gif';
			$disable_email = esc_html( get_option( 'settings_slack_notification_disable_email' ) );
			...
			$attachments = array(
				array(
					...
					'color'       => 'C03',
					'footer'      => esc_html__( 'WP Down Slack Alert, by Whodunit', 'wp-down-slack-alert' ),
